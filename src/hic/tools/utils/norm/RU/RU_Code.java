...
public class RU_Code() {
        public static int balance(long m, int[] i, int[] j, FloatObj x, double[] b, double[] report, double[] allIters, float tol, FloatObj pppp, int maxiter, double del, double dp, int totIter, int threads, int k) {
...
//	row is the current rows sum; dr and dc are the current rows and columns scaling vectors
                ...
                while((ber > tol || err > 5.0*tol) && iter < maxiter) {
                        ...
                        for (p=0;p<k;p++) {
                                if (bad[p] == 1) continue;
                                if (Math.abs((double) ((b[p]-current[p])/(b[p]+current[p]))) > ber) ber = Math.abs((double) ((b[p]-current[p])/(b[p]+current[p])));
                        }
...
//	iterations
                        if (iter % 10 == 0) {
                                ...
                                for (p=0;p<k;p++) {
                                        if (bad[p] == 1) continue;
                                        if (err < Math.abs((double) (row[p]*b[p] - one[p]))) err = Math.abs((double) (row[p]*b[p] - one[p]));
                                }
                                errors[nerr++] = err;
                        }
                        for (p=0;p<k;p++) current[p] = b[p];
